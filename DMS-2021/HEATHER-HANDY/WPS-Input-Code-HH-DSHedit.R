...
library(tidyverse)
library(here)
supersub <- read_csv(here("INPUT-FILES/supersub-homework-input.csv"))
...
###the first split using colon causes the rating to split together with the next sub-question, 
# which facilitates the need to split again by the first comma (which directly follows the rating)
###Each col group corresponds to the question groups that are being split to 
# respective sub questions and their ratings
...
HH_output <- read_csv(here("DMS-2021/HEATHER-HANDY/supersub-clean-input.csv"))
WPS_output <- read_csv(here("INPUT-FILES/supersub-homework-clean-input.csv"))
